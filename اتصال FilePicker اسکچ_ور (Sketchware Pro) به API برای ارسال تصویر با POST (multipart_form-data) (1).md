...
این کدها توابع و متغیرهای کمکی هستند که در کل اکتیویتی شما قابل دسترسی خواهند بود. آن‌ها را در بخش `Class Globals` قرار دهید.

**محل قرارگیری:**
در Sketchware Pro، به منوی سه‌نقطه (بالا سمت راست) بروید → «افزودن سورس مستقیم» (Add source directly) را انتخاب کنید → سپس تب «Class Globals» را باز کنید.
...
این مهم‌ترین بخش است که پس از انتخاب فایل توسط کاربر، فایل را برای ارسال به API آماده می‌کند و درخواست POST را می‌فرستد.
    ...
    try {
        ...
        runOnUiThread(() -> {
            try {
                ...
                linear11.setVisibility(android.view.View.GONE); // مخفی کردن linear11 (اگر برای نمایش وضعیت 
